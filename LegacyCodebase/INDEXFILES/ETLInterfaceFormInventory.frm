...
Begin VB.Form ETLInterfaceFormInventory 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "ETL(Accounts & Inventory) Connect Interface"
   ClientHeight    =   5445
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   8820
   ControlBox      =   0   'False
   Icon            =   "ETLInterfaceFormInventory.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form3"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame frmResult 
      ...
      Begin RichTextLib.RichTextBox RichTextBox1 
         ...
         ScrollBars      =   3
         Appearance      =   0
         TextRTF         =   $"ETLInterfaceFormInventory.frx":058A
      End
   ...
   Begin VB.ComboBox Combo1 
      Height          =   315
      Index           =   1
      ItemData        =   "ETLInterfaceFormInventory.frx":060C
      Left            =   1440
      List            =   "ETLInterfaceFormInventory.frx":0619
   ...
   Begin VB.ComboBox Combo1 
      Height          =   315
      Index           =   2
      ItemData        =   "ETLInterfaceFormInventory.frx":0638
      Left            =   1440
      List            =   "ETLInterfaceFormInventory.frx":0645
      Style           =   2  'Dropdown List
      TabIndex        =   24
      Top             =   2160
      Width           =   1815
   ...
   Begin VB.ComboBox Combo1 
      Height          =   315
      Index           =   0
      ItemData        =   "ETLInterfaceFormInventory.frx":0669
      Left            =   1440
      List            =   "ETLInterfaceFormInventory.frx":0673
      Style           =   2  'Dropdown List
      TabIndex        =   23
      Top             =   2160
      Width           =   1815
   ...
   Begin VB.ComboBox cmbDescription 
      Height          =   315
      ItemData        =   "ETLInterfaceFormInventory.frx":0681
      Left            =   1440
      List            =   "ETLInterfaceFormInventory.frx":06B8
      Style           =   2  'Dropdown List
      TabIndex        =   3
      Top             =   1200
      Width           =   3495
   ...
   End
End
Attribute VB_Name = "ETLInterfaceFormInventory"
...
'    cmbZephyrCompany.ListIndex = 0
    ...
    dtpFrom.Value = Format(Now, modGeneral.DtFormat)
    dtpTo.Value = Format(Now, modGeneral.DtFormat)
...
Private Sub cmbDescription_Click()
    ...
    If cmbDescription.ListIndex = 13 Then cmbDescription.ListIndex = 14
    ...
    ElseIf cmbDescription.ListIndex >= 8 And cmbDescription.ListIndex < 13 Then
        lblDate.Visible = True
        dtpFrom.Visible = True
        dtpTo.Visible = True
        Label4.Visible = True
        Combo1(2).Visible = True
    ElseIf cmbDescription.ListIndex = 14 Then
        lblDate.Visible = False
        dtpFrom.Visible = False
        dtpTo.Visible = False
        Label4.Visible = False
    ElseIf cmbDescription.ListIndex = 15 Then
        lblDate.Visible = False
        dtpFrom.Visible = False
        dtpTo.Visible = False
        Label4.Visible = False
        Combo1(0).Visible = True
    ElseIf cmbDescription.ListIndex = 16 Then
        lblDate.Visible = False
        dtpFrom.Visible = False
        dtpTo.Visible = False
        Label4.Visible = False
...
Private Sub cmdSend_Click()
    ...
    If Trim(RichTextBox1.Text) = "" Then
        frmResult.Visible = False
    End If
    MsgBox "ETL Transfer completed", vbInformation, "Zephyr"
    Me.Enabled = True
End Sub
...
ElseIf Combo1(2).Visible = True And cmbDescription.ListIndex >= 8 And cmbDescription.ListIndex < 14 Then
...
ElseIf Combo1(0).Visible = True And cmbDescription.ListIndex = 15 Then
    STR = IIf(Combo1(0).ListIndex = 0, "grp_cat", "grp_cls")
ElseIf Combo1(0).Visible = True And cmbDescription.ListIndex = 16 Then
    STR = IIf(Combo1(0).ListIndex = 0, "prd_idy", "prd_nme")
End If
...
'lvValues.ColumnHeaders.CLEAR
    ...
    ElseIf cmbDescription.ListIndex >= 3 And cmbDescription.ListIndex <= 7 Then
        SqlStatement = "select txn_idy AS ID, txn_dat AS TXN_DATE, txn_typ AS TXN_TYPE, txn_amt AS TXN_AMT, a1.act_nme as dr_acT, a2.act_nme as cr_ACT  " & _
                ...
                " AND txn_dat  >= to_date('" & FromDate & "', 'MM/DD/YYYY') " & _
                " AND txn_dat  <= to_date('" & ToDate & "', 'MM/DD/YYYY') "
        If STR <> "" Then SqlStatement = SqlStatement & " and " & STR & " like '" & txtFilter.Text & "%' "
         SqlStatement = SqlStatement & " order by txn_idy"
        FillListView lvValues, SqlStatement
    ElseIf cmbDescription.ListIndex >= 9 And cmbDescription.ListIndex <= 13 Then
        SqlStatement = "select INL.inv_idy, INL.INV_DAT, INL.txn_typ, sum(inv.col_amt) as prd_val, sum(INV.DIS_AMT) as prd_dis," & _
            ...
            " and INL.INV_DAT   >= to_date('" & FromDate & "', 'MM/DD/YYYY')  " & _
            " and INL.INV_DAT   <= to_date('" & ToDate & "', 'MM/DD/YYYY')   "  'and LENGTH(TRIM(RAT_APP))=1
            If STR <> "" Then SqlStatement = SqlStatement & " and " & STR & " like '" & txtFilter.Text & "%' "
        ...
        FillListView lvValues, SqlStatement
    ElseIf cmbDescription.ListIndex = 14 Then
        SqlStatement = "SELECT DISTINCT PRD_UOM FROM PRD ORDER BY PRD_UOM"
        FillListView lvValues, SqlStatement
'    ElseIf cmbDescription.ListIndex = 15 Then
'        SqlStatement = "SELECT DISTINCT PRD_UOM FROM PRD ORDER BY PRD_UOM"
'        FillListView lvValues, SqlStatement
...
On err GoTo ErrHandler
    ...
    If cmbDescription.ListIndex >= 0 And cmbDescription.ListIndex <= 2 Then
        If cmbDescription.ListIndex = 0 Then
    ...
    '-------create voucher opening balances for a specific balances
        ...
        End If
    End If
        If cmbDescription.ListIndex = 14 Then
            modProcess.CreateUOM STR
        ElseIf cmbDescription.ListIndex = 15 Then
            CreateStockGroup STR
        ElseIf cmbDescription.ListIndex = 16 Then
            Dim J As Integer
            J = 0
            STR = ""
...
'OPN - Opening stock
    ...
    If I >= 0 And I <= 6 Then
        ...
        End If
        STR = Mid(STR, 1, Len(STR) - 1)
        modProcess.CreateInventoryVoucherWithAcc IVchType(cmbDescription.ListIndex - 8), STR
    End If
'ErrHandler:
End Function
