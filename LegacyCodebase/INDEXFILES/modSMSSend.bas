...
'6-Retailer
    ...
    If AccountOther = 1 Then
        If GetBy = 0 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & " act.mar_idy in  (select mar_idy from mar where mar_nme like " & """" & Trim(Value) & "%" & """" & " )"
        If GetBy = 1 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & "  act.smn_idy  in (select smn_idy from smn where smn_nme like '" & Trim(Value) & "%' "
        If GetBy = 2 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & "  act.rou_idy like '" & Trim(Value) & "%' "
        If GetBy = 3 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & "  act.loc_idy in (select loc_idy from loc where loc_nme like '" & Trim(Value) & "%' "
        If GetBy = 4 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & "  act.cat_idy in (select cat_idy from cat where cat_nme like '" & Trim(Value) & "%' "
        If GetBy = 5 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & "  act.act_typ = 'PUR' and act.act_nme like '" & Trim(Value) & "%' "
        If GetBy = 6 And Trim(Value) <> "" Then SqlStatement1 = SqlStatement1 & IIf(InStr(1, UCase(SqlStatement1), "WHERE") > 0, " AND ", " WHERE ") & "  act.act_typ = 'SAL' and act.act_nme like '" & Trim(Value) & "%' "
    End If
