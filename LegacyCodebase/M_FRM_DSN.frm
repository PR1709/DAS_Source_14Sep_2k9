...
If M_FRM_DSN.Tag = "NRQ" Then
    Dim NRA_QRY As String
    NRA_QRY = "ACT->CST_TYP <> 'Van' .AND. ACT->ACT_DOC >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. ACT->ACT_DOC <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .and. ACT_TYP = 'SAL'"
    OpenReport CURDIR & "\LISTS.RP1", "Retailers List ID", 2, NRA_QRY, 6
ElseIf M_FRM_DSN.Tag = "Sales Return Report" Then
    SAL_QRY = SAL_QRY & ".and.inv->txn_typ='SRN'"
    OpenReport CURDIR & "\report.RP1", "Sales Return Report", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "Adjustment" Then
...
ElseIf Me.Tag = "Damage CNV" Then
    ...
    OpenReport CURDIR & "\Vouch.RP1", "Stock Conversion Report", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "SAL_PRD_TAX_TTL" Then
    SAL_QRY = SAL_QRY & ".and.inv->txn_typ='SAL'"
    OpenReport CURDIR & "\REPORT.RP1", "Sales Product Tax Total", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "PUR_VAT" Then
    'SAL_QRY = SAL_QRY & ".and. (INV->TXN_TYP='SAL' .or. INV->TAX_TYP='PRT')"
    SAL_QRY = SAL_QRY & ".and. (INV->TXN_TYP='SAL' .or. INV->TXN_TYP='PRT' .and. INV->STK_TYP<>'Free'.and. INV->STK_TYP<>'Replc.'.and. INV->STK_TYP<>'TakeBack'.and. INV->STK_TYP<>'Damage'.and. INV->STK_TYP<>'Shortage'.and. INV->STK_TYP<>'Breakage'.and. INV->STK_TYP<>'Leakage' )" 'CHANGED BY PKB
    OpenReport CURDIR & "\REPORT.RP1", "Sales Product Tax VAT", 2, Trim(SAL_QRY), 6
    OpenReport CURDIR & "\REPORT.RP1", "Sales PCG VAT Report", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "SAL_VAT" Then
    SAL_QRY = SAL_QRY & ".and. (INV->TXN_TYP='PUR' .or. INV->TXN_TYP='OPS' .or. INV->TXN_TYP='SRN')"
    OpenReport CURDIR & "\REPORT.RP1", "Purchase Product Tax VAT", 2, Trim(SAL_QRY), 6
    OpenReport CURDIR & "\REPORT.RP1", "Purchase PCG VAT Report", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "VAT_CAL" Then
    SAL_QRY = SAL_QRY & ".and. (INV->TXN_TYP='SAL' .or. INV->TXN_TYP='PUR' .or. INV->TXN_TYP='OPS' .or. INV->TXN_TYP='SRN' .or. INV->TXN_TYP='PRT')"
    OpenReport CURDIR & "\REPORT.RP1", "Tax Calculation Report VAT", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "UOM_SAL" Then
...
ElseIf M_FRM_DSN.Tag = "SCH_REF" Then
    Dim BRG_SCH As New ADODB.Recordset
    If BRG_SCH.State = 1 Then BRG_SCH.Close
    BRG_SCH.Open "select INV_IDY from INV where STK_TYP = 'Free' and INV_DAT >= CTOD('" & Format(DTPicker1, "MM/DD/YYYY") & "') AND inv_dat <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "')", con, adOpenKeyset, adLockPessimistic
    con.Execute "delete from TOP"
    While BRG_SCH.EOF = False
        con.Execute "INSERT INTO TOP VALUES('" & Trim(BRG_SCH(0) & "") & "'," & 0 & "," & 0 & ")"
        BRG_SCH.MoveNext
    Wend
    SAL_QRY = "INL->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INL->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .AND. LEN(TRIM(LEV_DES)) > 0"
     '.AND. LNV->INV_GRS > 0 query changed by PNR for Alankar Agency problem
    OpenReport CURDIR & "\REPORT.RP1", "Sales Scheme Date SKU", 2, SAL_QRY, 6
    OpenReport CURDIR & "\REPORT.RP1", "Sales Scheme SKU Smry", 2, SAL_QRY, 6
    OpenReport CURDIR & "\REPORT.RP1", "Sales Scheme Date SKU Smry", 2, SAL_QRY, 6
    SAL_QRY = "INL->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INL->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .AND. LNV->LNV_AMT > 0"
     '.AND. LNV->LNV_AMT > 0 query changed by PNR for Alankar Agency problem - RESET DONE BACK TO ORIGINAL CODE, SINCE THE EXACT PROBLEM COULD NOT BE IDENTIFIED YET
    OpenReport CURDIR & "\REPORT.RP1", "Sales Scheme Value SKU", 2, SAL_QRY, 6
    M_FRM_AQY.Tag = "CLJ"
    M_FRM_AQY.Show 1
ElseIf M_FRM_DSN.Tag = "SCH_QTY_MER" Then
    Scheme_Column_Update
    SAL_QRY = "INL->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INL->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .AND. LNV->LNV_AMT = 0 .AND. SCH->CMP_DST = 'N'"
    OpenReport CURDIR & "\KOR.RP1", "Sales Scheme Footer Smry MER", 2, SAL_QRY, 6
    SAL_QRY = "INL->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INL->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .AND. LNV->LNV_AMT = 0 .AND. SCH->CMP_DST = 'Y'"
    OpenReport CURDIR & "\KOR.RP1", "Sales Scheme Footer Smry MER B", 2, SAL_QRY, 6
    SAL_QRY = "INL->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INL->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .AND. LNV->LNV_AMT = 0"
    OpenReport CURDIR & "\KOR.RP1", "Sales Scheme Date SKU Smry MER", 2, SAL_QRY, 6
    SAL_QRY = "INL->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INL->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "')"
    OpenReport CURDIR & "\KOR.RP1", "Sales Scheme Invoice MER", 2, SAL_QRY, 6
ElseIf M_FRM_DSN.Tag = "SCH_VAL_MER" Then
    SAL_QRY = "INV->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. INV->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "')  .AND. INV->TXN_TYP = 'SAL' .AND. (LNV->LNV_AMT > 0)"
    OpenReport CURDIR & "\KOR.RP1", "Sales Scheme MER Consolidated", 2, SAL_QRY, 6
ElseIf M_FRM_DSN.Tag = "EMP_MOV_RPT" Then
    Empty_Stock_Movement DTPicker1, DTPicker2
    OpenReport CURDIR & "\KOR.RP1", "Empties Movement Report", 2, "", 6
ElseIf M_FRM_DSN.Tag = "PUR_PRD_TAX_TTL" Then
    SAL_QRY = SAL_QRY & ".and.inv->txn_typ='PUR'"
    OpenReport CURDIR & "\REPORT.RP1", "Purchase Product Tax Total", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "Sales Return Product" Then
    SAL_QRY = SAL_QRY & ".and.inv->txn_typ='SRN'"
    OpenReport CURDIR & "\report.RP1", "Sales Return Product", 2, Trim(SAL_QRY), 6
ElseIf M_FRM_DSN.Tag = "Tax Report" Then
...
ElseIf Me.Tag = "EMP_REG" Then
    OpenReport CURDIR & "\SNN.RP1", "Empties Register", 2, "SNV->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "') .and. SNV->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .and. (ACT->CST_TYP <> 'Van') .and. (SNV->TXN_TYP <> 'SAL')", 6
ElseIf Me.Tag = "NSPD" Then
    SAL_QRY = SAL_QRY & ".AND. (INV->STK_TYP='Breakage'.or.INV->STK_TYP='Leakage'.or.INV->STK_TYP='Damage'.or.INV->STK_TYP='Shortage')"
    OpenReport CURDIR & "\REPORT.RP1", "Non Salable Product Details", 2, SAL_QRY, 6
ElseIf Me.Tag = "NSPD_VAN" Then
    SAL_QRY = "VAN->INV_DAT >= ctod('" & Format(DTPicker1, "MM/DD/YYYY") & "').and. VAN->INV_DAT <= ctod('" & Format(DTPicker2, "MM/DD/YYYY") & "') .and. (BRK_QTY > 0 .or. LKG_QTY > 0)"
    OpenReport CURDIR & "\REPORT.RP1", "Non Salable Product VAN", 2, SAL_QRY, 6
ElseIf Me.Tag = "SSM" Or Me.Tag = "SUN" Or Me.Tag = "PRD" Or Me.Tag = "Inventory" Or Me.Tag = "COK_STK_BAT" Or Me.Tag = "COK_STK_BRD" Or Me.Tag = "COK_STK_SEG" Or Me.Tag = "DIS_STK_REP" Then
    If rs.State = 1 Then rs.Close
    ...
    If Me.Tag = "SUN" Then OpenReport CURDIR & "\stock.RP1", "Stock Summary Cases", 0, "", 6
    ...
    'If Me.Tag = "DIS_STK_REP" Then Openreport CURDIR & "\vvr_rpt.RP1", "Stock Summary Cases Van", 0, "", 6
...
ElseIf Me.Tag = "LOGREP" Then
    ...
    'M_REP_IQY.Tag = M_FRM_DSN.Tag
    'M_REP_IQY.Show 1
End If
If Me.Tag = "stkStmt" Then
    stkStmt DTPicker1.Value, DTPicker2.Value
End If
If Me.Tag = "VAN_CNQ" Then
   OpenReport CURDIR & "\vvr_rpt.RP1", "Van Cash n Cheque Report", 2, "VAN->INV_DAT>=CTOD('" & Format(DTPicker1, "MM/DD/YYYY") & "').AND.VAN->INV_DAT<=CTOD('" & Format(DTPicker2, "MM/DD/YYYY") & "')", 6
End If
Unload Me
Set Form = Nothing
If M_FRM_DSN.Caption = "Stock Ledger" Then
Unload Me
Set Form = Nothing
End If
End Sub
...
rs.Open "SELECT * FROM SCH WHERE SCH_IDY IN(SELECT SCH_IDY FROM INV WHERE inv_dat >= ctod('" & DTPicker1 & "') AND INV_DAT <=CTOD('" & DTPicker2 & "') GROUP BY SCH_IDY) and sch_typ not in('Pack Size','Pack Type','Brand','Segment','Category') GROUP BY SCH_IDY,B_PRD_IDY,TXN_TYP ", con, adOpenKeyset, adLockPessimistic
For A = 1 To rs.RecordCount
    RS1.AddNew
    RS1(0) = Date
...
RS1.Open "SELECT * FROM SCL", con, adOpenKeyset, adLockPessimistic
rs.Open "SELECT * FROM SCH WHERE SCH_IDY IN(SELECT SCH_IDY FROM INV WHERE inv_dat >= ctod('" & DTPicker1 & "') AND INV_DAT <=CTOD('" & DTPicker2 & "') GROUP BY SCH_IDY) and sch_typ in('Pack Size','Pack Type','Brand','Segment','Category') GROUP BY SCH_IDY,B_PRD_IDY,TXN_TYP ", con, adOpenKeyset, adLockPessimistic
For A = 1 To rs.RecordCount
    If Trim(rs!SCH_TYP) = "Pack Size" Then
        If rs!TXN_TYP = "BAS" Then
            If rs2.State = 1 Then rs2.Close
            rs2.Open "select * from prd where prd_pck = '" & Trim(rs!grp_typ) & "'", con, adOpenKeyset, adLockPessimistic
            Do While rs2.EOF = False
                RS1.AddNew
                RS1(0) = Date
                RS1(2) = Trim(rs2!prd_idy & "")
                RS1(1) = Trim(rs!Sch_Idy)
                RS1(3) = 0
                RS1(4) = 0
                RS1(5) = 0
        ...
        ElseIf rs!TXN_TYP = "FRE" Then
                ...
                RS1.Update
        End If
    ElseIf Trim(rs!SCH_TYP) = "Pack Type" Then
        If rs!TXN_TYP = "BAS" Then
            If rs2.State = 1 Then rs2.Close
            rs2.Open "select * from prd where prd_pkt = '" & Trim(rs!grp_typ) & "'", con, adOpenKeyset, adLockPessimistic
            Do While rs2.EOF = False
                RS1.AddNew
                RS1(0) = Date
                RS1(2) = Trim(rs2!prd_idy & "")
        ...
        ElseIf rs!TXN_TYP = "FRE" Then
                ...
                RS1.Update
        End If
    ElseIf Trim(rs!SCH_TYP) = "Brand" Then
        If rs!TXN_TYP = "BAS" Then
            If rs2.State = 1 Then rs2.Close
            rs2.Open "select * from prd where brd_nme = '" & Trim(rs!grp_typ) & "'", con, adOpenKeyset, adLockPessimistic
            Do While rs2.EOF = False
                RS1.AddNew
                RS1(0) = Date
                RS1(2) = Trim(rs2!prd_idy & "")
        ...
        ElseIf rs!TXN_TYP = "FRE" Then
                ...
                RS1.Update
        End If
    ElseIf Trim(rs!SCH_TYP) = "Segment" Then
        If rs!TXN_TYP = "BAS" Then
            If rs2.State = 1 Then rs2.Close
            rs2.Open "select * from pgr,prd where prd.grp_idy like pgr.grp_idy and pgr.grp_clf = '" & Trim(rs!grp_typ) & "'", con, adOpenKeyset, adLockPessimistic
            Do While rs2.EOF = False
                RS1.AddNew
                RS1(0) = Date
                RS1(2) = Trim(rs2!prd_idy & "")
        ...
        ElseIf rs!TXN_TYP = "FRE" Then
                ...
                RS1.Update
        End If
    ElseIf Trim(rs!SCH_TYP) = "Category" Then
        If rs!TXN_TYP = "BAS" Then
            If rs2.State = 1 Then rs2.Close
            rs2.Open "select * from pgr,prd where prd.grp_idy like pgr.grp_idy and pgr.grp_cat = '" & Trim(rs!grp_typ) & "'", con, adOpenKeyset, adLockPessimistic
            Do While rs2.EOF = False
                RS1.AddNew
                RS1(0) = Date
                RS1(2) = Trim(rs2!prd_idy & "")
...
Next
    ...
    ElseIf Me.Tag = "Claims Report Brand" Then
        ...
        For A = 1 To RS1.RecordCount
            ...
            RS1!fre_stk = Val(RS1!fre_stk & "") * Round((Val(SUBUNT)), 2)
            RS1!sal_stk = Val(RS1!sal_stk & "") * Round(Val(SUBUNT), 2)
            RS1.Update
            RS1.MoveNext
        Next
        If rs.State = 1 Then rs.Close
...
Loop
If rs.State = 1 Then rs.Close
rs.Open "SELECT COUNT(*),RDY.SMN_IDY,RDY.DAY_IDY FROM RDY,ACT WHERE ACT.ACT_IDY = RDY.ACT_IDY AND ACT.DBT_TYP = 'Y' GROUP BY RDY.DAY_IDY, RDY.SMN_IDY", con, adOpenKeyset, adLockPessimistic
...
Private Sub ADDRP1(smnid As String, CTR As Long)
    ...
    rs.Update
End Sub
Private Sub Empty_Stock_Movement(FRM_DAT As Date, TO_DAT As Date)
Dim RP1_TBL As New ADODB.Recordset
Dim SNV_TXN As New ADODB.Recordset
Dim CHK_SUP As New ADODB.Recordset
Dim RP2_TBL As New ADODB.Recordset

If RP2_TBL.State = 1 Then RP2_TBL.Close
con.Execute "DELETE FROM RP2"
RP2_TBL.Open "SELECT * FROM RP2", con, adOpenKeyset, adLockPessimistic
RP2_TBL.AddNew
RP2_TBL!CHR_ONE = "Shell"
RP2_TBL!CHR_TWO = ""
RP2_TBL!CHR_THR = ""
RP2_TBL!CHR_FOR = ""
RP2_TBL!CHR_FIV = ""
...
RP2_TBL.Update
If RP2_TBL.State = 1 Then RP2_TBL.Close
RP2_TBL.Open "SELECT * FROM RP2 WHERE CHR_ONE LIKE 'Shell'", con, adOpenKeyset, adLockPessimistic
If RP2_TBL.RecordCount > 0 Then RP2_TBL.MoveFirst
' *
If RP1_TBL.State = 1 Then RP1_TBL.Close
con.Execute "DELETE FROM RP1"
RP1_TBL.Open "SELECT * FROM RP1", con, adOpenKeyset, adLockPessimistic
'*
If SNV_TXN.State = 1 Then SNV_TXN.Close
SNV_TXN.Open "SELECT * FROM SNV GROUP BY PRD_PCK", con, adOpenKeyset, adLockPessimistic
If SNV_TXN.RecordCount > 0 Then
    While SNV_TXN.EOF = False
        RP1_TBL.AddNew
        RP1_TBL!CHR_ONE = Trim(SNV_TXN("PRD_PCK"))
        RP1_TBL!NUM_ONE = 0#
        RP1_TBL!NUM_TWO = 0#
        RP1_TBL!NUM_THR = 0#
        RP1_TBL!NUM_FOR = 0#
        ...
        RP1_TBL!NUM_EIG = 0#
        RP1_TBL!NUM_NIN = 0#
        RP1_TBL!NUM_TEN = Trim(SNV_TXN("PRD_UPC"))
        RP1_TBL.Update
        SNV_TXN.MoveNext
    Wend
End If
' *
If SNV_TXN.State = 1 Then SNV_TXN.Close
SNV_TXN.Open "SELECT * FROM SNV WHERE INV_DAT < ctod('" & FRM_DAT & "')", con, adOpenKeyset, adLockPessimistic
  If SNV_TXN.RecordCount > 0 Then
        While SNV_TXN.EOF = False
            'RP1_TBL.Find "CHR_ONE LIKE '" & Trim(SNV_TXN("PRD_PCK")) & "'"
            If RP1_TBL.State = 1 Then RP1_TBL.Close
            RP1_TBL.Open "SELECT * FROM RP1 WHERE CHR_ONE LIKE '" & Trim(SNV_TXN("PRD_PCK")) & "'"
            If RP1_TBL.EOF = False Then
                If SNV_TXN!TXN_TYP = "AIN" Or SNV_TXN!TXN_TYP = "DPC" Or SNV_TXN!TXN_TYP = "STO" Then  'Or SNV_TXN!TXN_TYP = "SAL"
                    RP1_TBL!NUM_ONE = Val(RP1_TBL!NUM_ONE) + Val(SNV_TXN!d_pck_nos)
                    RP2_TBL!NUM_ONE = Val(RP2_TBL!NUM_ONE) + Val(SNV_TXN!d_shl_nos)
                End If
                If SNV_TXN!TXN_TYP = "AOU" Or SNV_TXN!TXN_TYP = "DPR" Then
                    RP1_TBL!NUM_ONE = (Val(RP1_TBL!NUM_ONE) - Val(SNV_TXN!d_pck_nos))
                    RP2_TBL!NUM_ONE = Val(RP2_TBL!NUM_ONE) - Val(SNV_TXN!d_shl_nos)
                End If
                If SNV_TXN!TXN_TYP = "VAN" Then
                    RP1_TBL!NUM_ONE = Val(RP1_TBL!NUM_ONE) + Val(SNV_TXN!s_pck_nos)
                    RP2_TBL!NUM_ONE = Val(RP2_TBL!NUM_ONE) + Val(SNV_TXN!s_shl_nos)
                End If
                RP1_TBL!NUM_ONE = (Val(RP1_TBL!NUM_ONE) - Val(SNV_TXN!BRK_PCK))
                RP2_TBL!NUM_ONE = Val(RP2_TBL!NUM_ONE) - Val(SNV_TXN!BRK_SHL)
                RP1_TBL.Update
                RP2_TBL.Update
            End If
            SNV_TXN.MoveNext
...
If SNV_TXN.State = 1 Then SNV_TXN.Close
SNV_TXN.Open "SELECT * FROM SNV WHERE INV_DAT >= ctod('" & FRM_DAT & "') AND INV_DAT <=CTOD('" & TO_DAT & "')", con, adOpenKeyset, adLockPessimistic
  If SNV_TXN.RecordCount > 0 Then
        While SNV_TXN.EOF = False
'            RP1_TBL.Find "CHR_ONE LIKE '" & Trim(SNV_TXN("PRD_PCK")) & "'"
            If RP1_TBL.State = 1 Then RP1_TBL.Close
            RP1_TBL.Open "SELECT * FROM RP1 WHERE CHR_ONE LIKE '" & Trim(SNV_TXN("PRD_PCK")) & "'"
            If RP1_TBL.EOF = False Then
                If SNV_TXN!TXN_TYP = "AIN" Then
                    RP1_TBL!NUM_TWO = Val(RP1_TBL!NUM_TWO) + Val(SNV_TXN!d_pck_nos)
                    RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) + Val(SNV_TXN!d_pck_nos)
                    RP2_TBL!NUM_TWO = Val(RP2_TBL!NUM_TWO) + Val(SNV_TXN!d_shl_nos)
                    RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) + Val(SNV_TXN!d_shl_nos)
                End If
                If SNV_TXN!TXN_TYP = "DPC" Then
                    RP1_TBL!NUM_THR = Val(RP1_TBL!NUM_THR) + Val(SNV_TXN!d_pck_nos)
                    RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) + Val(SNV_TXN!d_pck_nos)
                    RP2_TBL!NUM_THR = Val(RP2_TBL!NUM_THR) + Val(SNV_TXN!d_shl_nos)
                    RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) + Val(SNV_TXN!d_shl_nos)
                End If
                If SNV_TXN!TXN_TYP = "STO" Then 'SNV_TXN!TXN_TYP = "SAL" Or
                    RP1_TBL!NUM_FOR = Val(RP1_TBL!NUM_FOR) + Val(SNV_TXN!d_pck_nos)
                    RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) + Val(SNV_TXN!d_pck_nos)
                    RP2_TBL!NUM_FOR = Val(RP2_TBL!NUM_FOR) + Val(SNV_TXN!d_shl_nos)
                    RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) + Val(SNV_TXN!d_shl_nos)
                End If
                If SNV_TXN!TXN_TYP = "VAN" Then
                    RP1_TBL!NUM_FOR = Val(RP1_TBL!NUM_FOR) + Val(SNV_TXN!s_pck_nos)
                    RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) + Val(SNV_TXN!s_pck_nos)
                    RP2_TBL!NUM_FOR = Val(RP2_TBL!NUM_FOR) + Val(SNV_TXN!s_shl_nos)
                    RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) + Val(SNV_TXN!s_shl_nos)
                End If
                If SNV_TXN!TXN_TYP = "AOU" Then
                    If CHK_SUP.State = 1 Then CHK_SUP.Close
                    CHK_SUP.Open "SELECT * FROM ACT WHERE ACT_IDY LIKE '" & Trim(SNV_TXN("CST_IDY")) & "' AND TRIM(GRP_CLF) = 'Creditor'", con, adOpenKeyset, adLockPessimistic
                    If CHK_SUP.RecordCount > 0 Then
                        RP1_TBL!NUM_FIV = Val(RP1_TBL!NUM_FIV) + Val(SNV_TXN!d_pck_nos)
                        RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) - Val(SNV_TXN!d_pck_nos)
                        RP2_TBL!NUM_FIV = Val(RP2_TBL!NUM_FIV) + Val(SNV_TXN!d_shl_nos)
                        RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) - Val(SNV_TXN!d_shl_nos)
                    Else
                        RP1_TBL!NUM_SEV = Val(RP1_TBL!NUM_SEV) + Val(SNV_TXN!d_pck_nos)
                        RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) - Val(SNV_TXN!d_pck_nos)
                        RP2_TBL!NUM_SEV = Val(RP2_TBL!NUM_SEV) + Val(SNV_TXN!d_shl_nos)
                        RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) - Val(SNV_TXN!d_shl_nos)
                    End If
                End If
                If SNV_TXN!TXN_TYP = "DPR" Then
                    RP1_TBL!NUM_EIG = Val(RP1_TBL!NUM_EIG) + Val(SNV_TXN!d_pck_nos)
                    RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) - Val(SNV_TXN!d_pck_nos)
                    RP2_TBL!NUM_EIG = Val(RP2_TBL!NUM_EIG) + Val(SNV_TXN!d_shl_nos)
                    RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) - Val(SNV_TXN!d_shl_nos)
                End If
                RP1_TBL!NUM_SIX = Val(RP1_TBL!NUM_SIX) + Val(SNV_TXN!BRK_PCK)
                RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_NIN) - Val(SNV_TXN!BRK_PCK)
                RP2_TBL!NUM_SIX = Val(RP2_TBL!NUM_SIX) + Val(SNV_TXN!BRK_SHL)
                RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_NIN) - Val(SNV_TXN!BRK_SHL)
                RP1_TBL.Update
                RP2_TBL.Update
            End If
            SNV_TXN.MoveNext
...
RP2_TBL.Open "SELECT * FROM RP2 WHERE CHR_ONE LIKE 'Shell'", con, adOpenKeyset, adLockPessimistic
If RP2_TBL.RecordCount > 0 Then RP2_TBL.MoveFirst
RP2_TBL!NUM_NIN = Val(RP2_TBL!NUM_ONE) + Val(RP2_TBL!NUM_NIN)
RP2_TBL.Update

If RP1_TBL.State = 1 Then RP1_TBL.Close
RP1_TBL.Open "SELECT * FROM RP1", con, adOpenKeyset, adLockPessimistic
If RP1_TBL.RecordCount > 0 Then
    While RP1_TBL.EOF = False
        RP1_TBL!NUM_NIN = Val(RP1_TBL!NUM_ONE) + Val(RP1_TBL!NUM_NIN)
        If RP1_TBL!NUM_ONE <> 0 Then
            RP1_TBL!NUM_ONE = STM(Val(RP1_TBL!NUM_ONE), Val(RP1_TBL!NUM_TEN))
            RP1_TBL.Update
        End If
        If RP1_TBL!NUM_TWO <> 0 Then
            RP1_TBL!NUM_TWO = STM(Val(RP1_TBL!NUM_TWO), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_THR <> 0 Then
            RP1_TBL!NUM_THR = STM(Val(RP1_TBL!NUM_THR), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_FOR <> 0 Then
            RP1_TBL!NUM_FOR = STM(Val(RP1_TBL!NUM_FOR), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_FIV <> 0 Then
            RP1_TBL!NUM_FIV = STM(Val(RP1_TBL!NUM_FIV), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_SIX <> 0 Then
            RP1_TBL!NUM_SIX = STM(Val(RP1_TBL!NUM_SIX), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_SEV <> 0 Then
            RP1_TBL!NUM_SEV = STM(Val(RP1_TBL!NUM_SEV), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_EIG <> 0 Then
            RP1_TBL!NUM_EIG = STM(Val(RP1_TBL!NUM_EIG), Val(RP1_TBL!NUM_TEN))
        End If
        If RP1_TBL!NUM_NIN <> 0 Then
            RP1_TBL!NUM_NIN = STM(Val(RP1_TBL!NUM_NIN), Val(RP1_TBL!NUM_TEN))
        End If
        RP1_TBL.Update
        RP1_TBL.MoveNext
    Wend
End If
End Sub
Private Sub Scheme_Column_Update()
Me.MousePointer = vbHourglass
On Error Resume Next
Dim LNV_RS As New ADODB.Recordset
Dim BRD_RS As New ADODB.Recordset
'If BRD_RS.State = 1 Then BRD_RS.Close
'BRD_RS.Open ("SELECT * FROM BRD"), CON, adOpenKeyset, adLockPessimistic

If LNV_RS.State = 1 Then LNV_RS.Close
LNV_RS.Open "SELECT * FROM LNV WHERE TXN_TYP = 'FRE' .AND. TRIM(LEV_DES) <> SPACE(10) .AND. INV_GRS <> 0", con, adOpenKeyset, adLockOptimistic
If LNV_RS.RecordCount > 0 Then LNV_RS.MoveFirst

'For A = 1 To LNV_RS.RecordCount
 While Not LNV_RS.EOF
    ...
    If BRD_RS.RecordCount > 0 Then
        'BRD_RS.MoveFirst
        'BRD_RS.Find "BAT_IDY LIKE '" & Trim(LNV_RS!LEV_DES) & "'"
        'If BRD_RS.EOF = False Then
