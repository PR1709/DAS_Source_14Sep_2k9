...
Dim DIR_RS As New ADODB.Recordset
  If DIR_RS.State = 1 Then DIR_RS.Close
    ...
    'Zip File Creation
            ...
            Dim FbkPath As String
            Dim Line_Count As Integer
            Dim Input_Data As Variant
            FbkPath = CurDir & "\FBKPATH.TXT"
            
            If Dir(CurDir & "\FBKPATH.TXT") = "" Then
                    M_FIL_NME = "C:\DEX\" & "FBKPDAS" & Trim(M_WDS_IDY) & Format(Date, "YYMMDD") & Format(Time, "HHMMSS") & ".ZIP"
            Else
                Open FbkPath For Input As #1
                If EOF(1) Then
                    M_FIL_NME = "C:\DEX\" & "FBKPDAS" & Trim(M_WDS_IDY) & Format(Date, "YYMMDD") & Format(Time, "HHMMSS") & ".ZIP"
                Else
                    Do
                    ...
                    Close #1
                    M_FIL_NME = Input_Data & "\" & "FBKPDAS" & Trim(M_WDS_IDY) & Format(Date, "YYMMDD") & Format(Time, "HHMMSS") & ".ZIP"
                End If
            End If
'            MsgBox FbkPath
'            MsgBox Input_Data
'            MsgBox M_FIL_NME
            ...
            If Dir(CurDir & "\DIR_INF.TXT") = "" Then
                ...
                F_VER.WriteLine "Text File   : " & M_FIL_NME
                F_VER.Close
            End If
...
'            Myzip.RemoveFile (CurDir & "\PNV.DBF")
'            Myzip.RemoveFile (CurDir & "\PNL.DBF")
            
'            If Dir("C:\DEX\OutBox\*.ZIP") = "" Then
'            Else
'                Myzip.AddFile ("C:\DEX\OutBox\*.ZIP")
'            End If
            ...
            'If MsgBox("Send Zip File Data to Help Desk Now ?", vbQuestion + vbYesNo, "DAS Version 4.0") = vbYes Then
                ...
                Else
                    MailSend "das@apac.ko.com", Trim(M_MSG_SUB), Trim(M_MSG_STR), Trim(M_MAIL_ATTMNT)
                End If
