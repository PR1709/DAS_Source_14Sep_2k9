...
Begin VB.Form CMP_SEL 
   ...
   Caption         =   "DAS Version 4.0 DASXtract DB Utility"
   ClientHeight    =   1815
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5205
...
If Len(Trim(Combo1)) = 0 Then
    MsgBox "Please Select Company Name !", vbExclamation, "DAS Version 4.0"
    Combo1.SetFocus
Else
    If SysInfo1.OSPlatform = 1 Then
        Open "c:\windows\system\EFFMCG4.txt" For Input As #1   'Open file for output.
...
If App.PrevInstance = True Then
    MsgBox "DAS Version 4.0 Index Application Already Open or In Process !", vbCritical, "DAS Version 4.0"
    End
    Exit Sub
End If
    If SysInfo1.OSPlatform = 1 Then
...
' * : Beginning of DAS_STOCK.DAT Creation Code for Data Transfer
    ...
    Do While LDD <> 0
        ...
        '            F_STK.WriteLine Trim(M_WDS_IDY) & "|" & Trim(L_PRD_COD) & "|" & Trim(STR(CLD_RS("OPN_STK"))) & "|" & Trim(STR(CLD_RS("PUR_STK"))) & "|" & Trim(STR(CLD_RS("PRT_STK"))) & "|" & Trim(STR(CLD_RS("SAL_STK"))) & "|" & Trim(STR(CLD_RS("SRN_STK"))) & "|" & Trim(STR(CLD_RS("STI_STK"))) & "|" & Trim(STR(CLD_RS("STO_STK"))) & "|" & Trim(STR(CLD_RS("CLS_STK"))) & "|" & Trim(STR(BAT_RS("PRD_MRP"))) & "|" & Trim(BAT_RS("BAT_NME")) & "|" & BAT_RS("BRD_DOM")
                    ...
                    F_STK.WriteLine Format(BEG_DAT, "YYYYMMDD") & "|" & Format(END_DAT, "YYYYMMDD") & "|" & Trim(L_PRD_COD) & "|" & Trim(M_WDS_IDY) & "|" & Val(CLD_RS(1)) & "|" & amt(Val(CLD_RS(1)) / Val(PRD_RS("SUB_UNT")))
                    CLD_RS.MoveNext
                    l = l + 1
                Wend
